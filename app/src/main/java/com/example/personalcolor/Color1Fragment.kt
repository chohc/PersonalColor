...
class Color1Fragment : Fragment() {

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        // 레이아웃 인플레이션
        val view = inflater.inflate(R.layout.fragment_color1, container, false)
        // GridView 찾기
        val gridView: GridView = view.findViewById(R.id.color1GridView)
        // 이미지 리소스 설정
        ...
        gridView.setOnItemClickListener { _, _, _, _ ->
            ...
            test2Activity?.let {
                it.supportFragmentManager.beginTransaction()
                    .replace(R.id.container, Color2Fragment()) // 다음 프래그먼트로 전환
                    .commit()
            }
        }
